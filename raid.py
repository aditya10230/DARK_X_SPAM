...
from resources.data import RAID, REPLYRAID, DARKXSPAM
...
async def spam(e):
    ...
    if e.sender_id in SUDO_USERS:
        ...
        Dark = ("".join(e.text.split(maxsplit=1)[1:])).split(" ", 1)
        ...
        if len(Dark) == 2:
            ...
            g = a.id
            if int(g) in Dark:
                text = f"PAGAL HAI KYA BSDK OWNER PRR RAID NHI KRUNGA @darkxspam_spam_bot's Owner"
                await e.reply(text, parse_mode=None, link_preview=None )
            elif int(g) == OWNER_ID:
                text = f"YE HI BANAYA HAI MUJHE."
                await e.reply(text, parse_mode=None, link_preview=None )
            elif int(g) in SUDO_USERS:
                text = f"YE OWNER KA CHELA HAI."
                await e.reply(text, parse_mode=None, link_preview=None )
            else:
                c = a.first_name
           ...
           g = b.id       if int(g) in DARKXSPAM :
           
            text = f"BADE CHUTIYO HO BHAI OWNER PRR RAID DILWAOGE WAAH @darkxspam_spam_bot's Owner"
                await e.reply(text, parse_mode=None, link_preview=None )
            elif int(g) == OWNER_ID:
                text = f"YE BANAYA HAI MUJHE"
                await e.reply(text, parse_mode=None, link_preview=None )
            elif int(g) in SUDO_USERS:
                text = f"MERE OWNER KA CHELA."
                await e.reply(text, parse_mode=None, link_preview=None )
            else:
                c = b.first_name
                counter = int(Dark[0])
...
async def _(e):
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(e.text) > 11:
            ...
            if int(user_id) in Dark:
                text = f" can't raid on @darkx_spam_bot's Owner."
                await e.reply(text, parse_mode=None, link_preview=None )
...
async def _(event):
   ...
   if event.sender_id in SUDO_USERS:
         if event.text[0].isalpha() and e.text[0] in ("/", "#", "@", "!"):
            return await e.reply(usage, parse_mode=None, link_preview=None )
         Dark = "".join(event.text.split(maxsplit=1)[1:]).split(" ", 2)
         if len(Dark) == 3:
             user = str(Dark[2])
             a = await event.client.get_entity(user)
             e = a.id
             if int(e) in DARKXSPAM:
                    text = f"I can't raid on @darkx_spam_bot's Owner"
                    await event.reply(text, parse_mode=None, link_preview=None )
             elif int(e) == OWNER_ID:
                text = f"This guy is a owner Of this Bots."
                await event.reply(text, parse_mode=None, link_preview=None )
                 ...
                 counter = int(Dark[1])
                 sleeptimet = sleeptimem = float(Dark[0])
                 for _ in range(counter):
                      reply = random.choice(RAID)
                      caption = f"{username} {reply}"
                      async with event.client.action(event.chat_id, "typing"):
